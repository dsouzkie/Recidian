...
*(Note: As this runs on a free Render instance, please allow 30-50 seconds for the server to wake up from sleep).*
...
```mermaid
graph LR
    A[Razorpay UI] --> B(Create Order & Pay)
    B --> C{Refund Requested}
    C --> D[POST /request-refund]
    D --> E[FastAPI Interceptor]
    
    E --> F{Rule Layer}
    F -->|Trigger| G[FLAG: HIGH RISK]
    F -->|Pass| H[XGBoost Engine]
    
    H --> I[SHAP Explainer]
    I --> J{Score >= 0.69?}
    J -->|Yes| K[Webhook Alert]
...
5. **View the Dashboard**
   Navigate to `http://localhost:8000` (which auto-redirects to the futuristic dashboard).
